...
import static io.agora.openduo.Constants.CAMERA_UID;
import static io.agora.openduo.Constants.SCREEN_SHARE_UID;
...
//        }
        ...
        mLocalPreviewLayout.addView(surface);
    }
    ...
    private void setupRemoteView(int uid) {
        ...
        if(mSS) {
            ...
            mLocalPreviewLayout.addView(ssV, new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, VideoCanvas.RENDER_MODE_FIT));
        }else{
    ...
    private void joinChannel() {
        mRtcEngine.joinChannel(null, mChannel,"Extra Optional Data", CAMERA_UID); // if you do not specify the uid, we will generate the uid for you
    }
    ...
    public void onScreenSharingClicked(View view) {
        ...
        if (button.isSelected()) {
            mSSClient.start(getApplicationContext(), getResources().getString(R.string.private_app_id), null,
                    mChannel, SCREEN_SHARE_UID, mVEC);
